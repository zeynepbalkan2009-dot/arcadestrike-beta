 ...
 *
 * Beta-stable offline-safe version:
 * - No Prisma dependency during matchmaking
 * - No escrow locking during local dev
 * - In-memory queue fallback
 * - Fast quick-match pairing
 */
...
export class MatchmakingQueue {
  ...
  async initialize(): Promise<void> {
    this.matchTicker = setInterval(() => {
      this.processQueue().catch((err) => {
        console.error("[MATCHMAKING ERROR]", err);
      });
    }, MATCH_TICK_MS);

    await redisInfrastructure.subscribe(
      "arcadestrike:room-events",
      (event) => {
  ...
  async join(entry: QueueEntry): Promise<void> {
    console.log("[QUEUE JOIN]", entry.playerId, entry.wagerAmount);

    const existingQueueKey = await this.getPlayerQueueKey(
      entry.playerId
    );

    if (
      existingQueueKey ||
      this.queue.some((e) => e.playerId === entry.playerId)
    ) {
    ...
    const queuedEntry: QueueEntry = {
      ...entry,
      joinedAt: Date.now(),
      queueMode: entry.queueMode || "quick",
    };
    ...
    console.log(
      ...
      this.queue.length,
      this.queue.map(
        (p) => `${p.playerId}:${p.wagerAmount}`
      )
    );

    metrics.counter(
      "arcadestrike_matchmaking_join_total",
      "Players joining matchmaking",
      {
        mode: queuedEntry.queueMode,
      }
    );
  ...
  async leave(playerId: string): Promise<void> {
    ...
    const distributedEntry =
      await this.removeDistributedEntry(playerId);

    if (idx === -1 && !distributedEntry) {
      return;
    }
  ...
  async getPosition(playerId: string): Promise<number> {
    ...
    if (queueKey && redisInfrastructure.client) {
      const rank =
        await redisInfrastructure.client.zrank(
          queueKey,
          playerId
        );
  ...
  async getPendingMatch(
    playerId: string
  ): Promise<MatchTicket | undefined> {
    const redis = redisInfrastructure.client;

    if (redis) {
      const raw = await redis.get(
        `matchmaking:pending:${playerId}`
      );

      return raw
        ? (JSON.parse(raw) as MatchTicket)
        : undefined;
    }
  ...
  private async processQueue(): Promise<void> {
    ...
    for (let i = 0; i < this.queue.length; i++) {
      if (matched.has(this.queue[i].playerId)) {
        continue;
      }
      ...
      for (let j = i + 1; j < this.queue.length; j++) {
        if (matched.has(this.queue[j].playerId)) {
          continue;
        }
        ...
        if (this.canMatch(p1, p2, now)) {
          ...
          await this.removeDistributedEntry(
            p1.playerId,
            false
          );
  ...
  private canMatch(
    ...
    if (p1.wagerAmount !== p2.wagerAmount) {
      return false;
    }

    if (p1.queueMode === "quick") {
      return true;
    }

    const p1Wait =
      (now - p1.joinedAt) / 1000;

    const p2Wait =
      (now - p2.joinedAt) / 1000;
    ...
    const eloRange = Math.min(
      ELO_INITIAL_RANGE +
        ELO_EXPAND_RATE * maxWait,
      ELO_MAX_RANGE
    );

    return (
      Math.abs(p1.elo - p2.elo) <= eloRange
    );
  }
  ...
  private async createMatch(
    ...
    try {
      ...
      await redisInfrastructure.publish(
        "arcadestrike:room-events",
        {
          type: "match_found",
      ...
      metrics.counter(
        "arcadestrike_matches_created_total",
        "Matchmaking-created rooms",
        {
          mode: ticket.queueMode,
        }
      );
  ...
  dispose(): void {
    if (this.matchTicker) {
      clearInterval(this.matchTicker);
    }
  }
  ...
  private queueKey(
    ...
    return `matchmaking:queue:${
      entry.queueMode || "quick"
    }:${entry.currency}:${entry.wagerAmount}`;
  }

  private async addDistributedEntry(
    entry: QueueEntry
  ): Promise<void> {
    const redis = redisInfrastructure.client;
    ...
    await redis
      ...
      .set(
        `matchmaking:entry:${entry.playerId}`,
        JSON.stringify(entry),
        "EX",
        120
      )
      .set(
        `matchmaking:player:${entry.playerId}`,
        key,
        "EX",
        120
      )
  ...
  private async removeDistributedEntry(
    playerId: string,
    deleteEntry = true
    ...
    return raw
      ? (JSON.parse(raw) as QueueEntry)
      : undefined;
  }

  private async getPlayerQueueKey(
    playerId: string
  ): Promise<string | null> {
    return (
      (await redisInfrastructure.client?.get(
        `matchmaking:player:${playerId}`
      )) ?? null
    );
  }
  ...
  private async hydrateDistributedQueue(): Promise<void> {
    ...
    const keys = await redis.smembers(
      "matchmaking:queue-keys"
    );

    const seen = new Set(
      this.queue.map((entry) => entry.playerId)
    );
  ...
  private async storePendingMatch(
    ticket: MatchTicket
  ): Promise<void> {
    const redis = redisInfrastructure.client;
    ...
    await redis
      .multi()
      .set(
        `matchmaking:pending:${ticket.players[0]}`,
        JSON.stringify(ticket),
        "EX",
        60
      )
      .set(
        `matchmaking:pending:${ticket.players[1]}`,
        JSON.stringify(ticket),
        "EX",
        60
      )
