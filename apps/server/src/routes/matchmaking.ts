...
export function createMatchmakingRouter(): Router {
  ...
  const joinQueueHandler = async (req: Request, res: Response) => {
    ...
    const { wagerAmount, currency, queueMode = "quick" } = req.body as JoinQueueRequest;
    ...
    if (!VALID_WAGERS.has(wagerAmount)) {
      return res.status(400).json({
        ...
        validWagers: Array.from(VALID_WAGERS).map(w => ethers.formatEther(w)),
      });
    }
    if (currency !== "REAL" && currency !== "PROMO") {
      return res.status(400).json({ error: "currency must be REAL or PROMO" });
    }
    if (queueMode !== "quick" && queueMode !== "ranked") {
      return res.status(400).json({ error: "queueMode must be quick or ranked" });
    }
    if (!queueInstance) {
      return res.status(503).json({ error: "Matchmaking service unavailable" });
    }
    ...
    const balance = currency === "REAL" ? wallet.realCredits : wallet.promoCredits;
    ...
    try {
      ...
      log.info({ playerId, wagerAmount, currency, queueMode, position }, "Joined queue");
      ...
      if (err.message === "ALREADY_IN_QUEUE") {
        return res.status(409).json({ error: "Already in queue", code: "ALREADY_IN_QUEUE" });
      }
  ...
  router.post("/queue", requireAuth, rateLimitApi, joinQueueHandler);
  router.post("/queue/quick", requireAuth, rateLimitApi, (req, res) => {
    req.body = { ...req.body, queueMode: "quick" satisfies QueueMode };
    return joinQueueHandler(req, res);
  });
  router.post("/queue/ranked", requireAuth, rateLimitApi, (req, res) => {
    req.body = { ...req.body, queueMode: "ranked" satisfies QueueMode };
    return joinQueueHandler(req, res);
  });

  router.delete("/queue", requireAuth, rateLimitApi, async (req, res) => {
    const playerId = req.auth!.playerId;
    if (!queueInstance) return res.status(503).json({ error: "Matchmaking service unavailable" });
  ...
  router.get("/status", requireAuth, rateLimitApi, async (req, res) => {
    const playerId = req.auth!.playerId;
    if (!queueInstance) return res.json({ inQueue: false, matchFound: false });
    ...
    const position = await queueInstance.getPosition(playerId);
    if (position === 0) return res.json({ inQueue: false, matchFound: false });
  ...
  router.post("/accept", requireAuth, rateLimitApi, async (req, res) => {
    const playerId = req.auth!.playerId;
    const match = await queueInstance?.getPendingMatch(playerId);
    if (!match) return res.status(404).json({ error: "No pending match", code: "MATCH_NOT_FOUND" });
    return res.json({ status: "accepted", match });
  });

  router.post("/decline", requireAuth, rateLimitApi, async (req, res) => {
    const playerId = req.auth!.playerId;
    if (!queueInstance) return res.status(503).json({ error: "Service unavailable" });

    try {
      await queueInstance.leave(playerId);
      return res.json({ status: "declined", refunded: true });
    } catch {
      return res.status(500).json({ error: "Internal error" });
    }
  });
